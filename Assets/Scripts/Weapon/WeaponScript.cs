...
public class WeaponScript : MonoBehaviour
{
    ...
    private void Shoot()
    {
        if (bulletLeft > 0 && canShoot && !reloading)
        {
            ...
            {
                int newDamage = Random.Range(damage - 2, damage + 2);
                if(hit.collider.gameObject.GetComponent<EnemyBehavior>() != null)
                {
                    hit.collider.gameObject.GetComponent<EnemyBehavior>().TakeDamage(newDamage);
                }
                else if (hit.collider.gameObject.GetComponent<DestroyableObject>() != null)
                {
                    hit.collider.gameObject.GetComponent<DestroyableObject>().TakeDamage(newDamage);
                }
            ...
            if (hit.point == new Vector3(0, 0, 0))
            {
                ...
                bulletTrail.GetComponent<BulletScript>().setTargetPos(bulletSpawnPos.position + transform.TransformDirection(Vector3.forward * 100 + RandomSpread/10), false, null);
            ...
            else
            {
                ...
                {
                    bulletTrail.GetComponent<BulletScript>().setTargetPos(hit.point, false, null);
                }
                else
                {
                    bulletTrail.GetComponent<BulletScript>().setTargetPos(hit.point, true, hit.collider.gameObject);
                }
                GameObject bulletSound = Instantiate(shootSoundRef, transform.position, Quaternion.identity);
                bulletSound.GetComponent<shootSoundScript>().setSound(weaponSounds[1]);
            }
